...
function MovieShowConfig() {
    ...
    const handleSubmitMovieShow = async () => {
        setIsLoaded(false);
    ...
    const handleMovieShowClick = (o, closedSale) => {
        if (closedSale === 1) {
            setIsInfoPopup(true);
            return;
        }
        ...
        if (object.newItem) {
            for (let i = 0; i < newMovieShows.length; i += 1) {
                if (object.id === newMovieShows[i].id) {
                    const modifiedArr = [...newMovieShows];
                    modifiedArr.splice(i, 1);
                    setNewMovieShows([...modifiedArr]);
                }
            }
        } else {
            for (let i = 0; i < newMovieShows.length; i += 1) {
            ...
            {isLoaded && (
                <div className="conf-step__wrapper">
                    ...
                    <p className="conf-step__paragraph">
                        ...
                        {activeDate && <input type="date" value={activeDate} min={today} onChange={(e) => handleDateChange(e)} />}
                    </p>
                                    ...
                                    {newMovieShows && newMovieShows.map((movie) => (
                                        movie.hall_id === hall.id && (
                                            <div
                                                ...
                                                role="button"
                                                tabIndex={0}
                                                onKeyPress={(e) => { if (e.key === 'Enter') handleMovieShowClick(movie, hall.is_active); }}
                                                onClick={() => handleMovieShowClick(movie, hall.is_active)}
                    ...
                    <fieldset className="conf-step__buttons text-center">
                        ...
                        <input type="submit" value="Сохранить" className="conf-step__button conf-step__button-accent" onClick={handleSubmitMovieShow} />
                    </fieldset>
                </div>
            )}
        </>
