export default class Api {
    ...
    static async getItems(address) {
        return new Promise((resolve, reject) => {
            ...
            xhr.open('GET',`http://localhost:8080/api/${address}`);
    ...
    static async getShow(address, number) {
        return new Promise((resolve, reject) => {
            const xhr = new XMLHttpRequest();
            xhr.open('GET',`http://localhost:8080/api/${address}/${number}`);
            xhr.setRequestHeader('X-CSRF-TOKEN', window.csrfToken);
            xhr.addEventListener('load', () => {
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
    ...
    static async deleteItems(address, id) {
        return new Promise((resolve, reject) => {
            const xhr = new XMLHttpRequest();
            xhr.open('GET',`http://localhost:8080/api/${address}/${id}`);
            xhr.setRequestHeader('X-CSRF-TOKEN', window.csrfToken);
            xhr.addEventListener('load', () => {
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
    ...
    static async updateHallPrice(address, id, array, price, vipPrice) {
        return new Promise((resolve, reject) => {
            const params = new URLSearchParams();
    ...
    static async getMovie(address, date) {
        return new Promise((resolve, reject) => {
            const xhr = new XMLHttpRequest();
            xhr.open('GET',`http://localhost:8080/api/${address}/${date}`);
            xhr.setRequestHeader('X-CSRF-TOKEN', window.csrfToken);
            xhr.addEventListener('load', () => {
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
